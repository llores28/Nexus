# Enterprise Nexus Bootstrap — Strict Variant

You are Cascade inside Windsurf.

Mission: create a compliance-grade, security-first Nexus intelligent project operating system for this repository with traceable evidence, governance controls, and production-ready documentation.
...
Additionally, install full CLI toolkit skills from `nexus/`:
...
### G) Token efficiency + cross-IDE support
...
Activation policy (token-efficient defaults):
- `00-project-overview` => Always On (small, essential context)
- `01-security-and-secrets` => Always On (non-negotiable)
- `02-secure-coding-and-input-validation` => Always On (security, keep concise)
- `03-change-management-and-approvals` => Model Decision (loaded only when relevant)
- `04-testing-and-quality-gates` => Model Decision (loaded only when relevant)
- `05-incidents-and-oncall` => Model Decision (loaded only when relevant)
- `06-data-governance-and-privacy` => Model Decision (loaded only when relevant)
- `07-dependency-and-supply-chain` => Model Decision (loaded only when relevant)
...
## Phase 3 — AGENTS hierarchy + cross-IDE instructions
