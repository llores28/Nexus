...
## 1) Single page fetch
```
python nexus/cli/bs_cli.py scrape page <url> --format json
```

## 2) JSON API fetch
```
python nexus/cli/bs_cli.py scrape api <url> --format json
```

## 3) Extract links from a page
```
python nexus/cli/bs_cli.py scrape links <url> --format json
```

## 4) Crawl a documentation site
```
python nexus/cli/bs_cli.py scrape docs <url> --depth 2 --format json
```
