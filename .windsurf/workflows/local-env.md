...
// turbo
```
python nexus/cli/bs_cli.py prereqs --component docker --format json
```

## 2) Initialize Docker files (if needed)
```
python nexus/cli/bs_cli.py local-env init --format json
```

## 3) Build the image
```
python nexus/cli/bs_cli.py local-env build --format json
```

## 4) Start containers
```
python nexus/cli/bs_cli.py local-env up --format json
```
...
// turbo
```
python nexus/cli/bs_cli.py local-env validate --format json
```
...
## 8) Teardown
```
python nexus/cli/bs_cli.py local-env down --format json
```
