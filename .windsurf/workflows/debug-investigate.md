...
// turbo
```
python nexus/cli/bs_cli.py debug env --format json
```
// turbo
```
python nexus/cli/bs_cli.py debug ports --format json
```

## 3) Check dependencies
// turbo
```
python nexus/cli/bs_cli.py debug deps --format json
```

## 4) Trace error origin
```
python nexus/cli/bs_cli.py debug trace "<error message from step 1>"
```

## 5) Scan logs (if applicable)
```
python nexus/cli/bs_cli.py debug logs <log-path-or-directory>
```

## 6) Secrets scan (always run before any commit suggestion)
// turbo
```
python nexus/cli/bs_cli.py debug secrets-scan --format json
```
